    ...
    SdlPrinter.prototype.printType = function (table, otherTables) {
        ...
        var relationFields = otherTables
            ...
            return t.columns
                ...
                .map(function (c) { return ({ remoteColumn: c, remoteTable: t.name }); });
        })
            .reduce(function (acc, next) { return acc.concat(next); }, []);
        return "type " + this.capitalizeFirstLetter(table.name) + " @postgres(table: \"" + table.name + "\") {" + _.map(nativeFields, function (column) { return _this.printField(column); }).join('') + relationFields.map(function (field) { return _this.printRelationField(field); }).join('') + "\n}";
    };
    SdlPrinter.prototype.printRelationField = function (field) {
        return "\n  " + field.remoteTable + "s: [" + this.capitalizeFirstLetter(field.remoteTable) + "] @postgres(foreignColumn: \"" + field.remoteColumn.name + "\")";
    };
    SdlPrinter.prototype.printField = function (column) {
        return "\n  " + this.printFieldName(column) + ": " + this.printFieldType(column) + this.printFieldOptional(column) + this.printFieldDirective(column);
    };
